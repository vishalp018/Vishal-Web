...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      ...
      .then(
        () => { form.current.reset(); toast.success("Message sent successfully!", { theme: "dark" }); },
        () => toast.error("Failed to send message. Please try again.", { theme: "dark" })
      );
  };

  const inputClass =
    "w-full p-3.5 rounded-xl bg-dark/80 text-neutral-100 border border-neutral-800 focus:outline-none focus:border-neutral-600 focus:shadow-[0_0_0_3px_rgba(255,255,255,0.04)] transition-all placeholder:text-neutral-600";

  return (
    <section id="contact" className="flex flex-col items-center justify-center py-24 px-[12vw] md:px-[7vw] lg:px-[20vw]">
      <ToastContainer />
      <AnimatedSection className="w-full">
        <SectionHeading title="Contact" subtitle="Open to SDE, Full-Stack, and SDET opportunities — let's connect!" />
      </AnimatedSection>
      ...
      <AnimatedSection delay={0.2} className="w-full max-w-lg">
        <motion.div className="glass-card p-8">
          ...
          <form ref={form} onSubmit={sendEmail} className="flex flex-col space-y-4">
            ...
            />
            <motion.button type="submit" className="btn-primary w-full py-3.5 rounded-xl" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              Send Message
            </motion.button>
          </form>
        </motion.div>
